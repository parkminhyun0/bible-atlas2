...
- 사건 동선: 오른쪽 패널(`data/genesis-routes.geojson`). 창세기 인물 17개 동선.
  직선이 아니라 지형 최소비용경로로 그린다 — Tobler 도보 함수를 비용으로 쓰는
  8방향 다익스트라(`bake/routes.py`). 확실성을 선 모양으로 나눈다:
  실선 본문 명시 · 파선 추정 · 점선 가설.
- 패널: 왼쪽은 지도 설정, 오른쪽은 사건 동선. 데스크톱은 각각 손잡이로 밀어 넣고,
  모바일은 바텀시트로 올린다.
